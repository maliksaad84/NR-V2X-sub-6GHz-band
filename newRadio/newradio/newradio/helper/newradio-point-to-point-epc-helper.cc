#include <ns3/newradio-point-to-point-epc-helper.h>
#include <ns3/log.h>
...
NS_LOG_COMPONENT_DEFINE ("newradioPointToPointEpcHelper");

namespace newradio {

NS_OBJECT_ENSURE_REGISTERED (newradioPointToPointEpcHelper);


newradioPointToPointEpcHelper::newradioPointToPointEpcHelper ()
  : m_gtpuUdpPort (2152),
...
TypeId
newradioPointToPointEpcHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::newradioPointToPointEpcHelper")
    .SetParent<EpcHelper> ()
    .SetGroupName ("newradio")
    .AddConstructor<newradioPointToPointEpcHelper> ()
    .AddAttribute ("S1uLinkDataRate",
                   "The data rate to be used for the next S1-U link to be created",
                   DataRateValue (DataRate ("10Gb/s")),
                   MakeDataRateAccessor (&newradioPointToPointEpcHelper::m_s1uLinkDataRate),
                   MakeDataRateChecker ())
    .AddAttribute ("S1uLinkDelay",
                   "The delay to be used for the next S1-U link to be created",
                   TimeValue (Seconds (0.001)),
                   MakeTimeAccessor (&newradioPointToPointEpcHelper::m_s1uLinkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("S1uLinkMtu",
                   ...
                   UintegerValue (2000),
                   MakeUintegerAccessor (&newradioPointToPointEpcHelper::m_s1uLinkMtu),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("S1apLinkDataRate",
                   "The data rate to be used for the S1-AP link to be created",
                   DataRateValue (DataRate ("10Gb/s")),
                   MakeDataRateAccessor (&newradioPointToPointEpcHelper::m_s1apLinkDataRate),
                   MakeDataRateChecker ())
    .AddAttribute ("S1apLinkDelay",
                   "The delay to be used for the S1-AP link to be created",
                   TimeValue (MilliSeconds (15)),
                   MakeTimeAccessor (&newradioPointToPointEpcHelper::m_s1apLinkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("S1apLinkMtu",
                   "The MTU of the next S1-AP link to be created",
                   UintegerValue (10000),
                   MakeUintegerAccessor (&newradioPointToPointEpcHelper::m_s1apLinkMtu),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("X2LinkDataRate",
                   "The data rate to be used for the next X2 link to be created",
                   DataRateValue (DataRate ("100Gb/s")),
                   MakeDataRateAccessor (&newradioPointToPointEpcHelper::m_x2LinkDataRate),
                   MakeDataRateChecker ())
    .AddAttribute ("X2LinkDelay",
                   "The delay to be used for the next X2 link to be created",
                   TimeValue (Seconds (0.001)),
                   MakeTimeAccessor (&newradioPointToPointEpcHelper::m_x2LinkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("X2LinkMtu",
                   ...
                   UintegerValue (10000),
                   MakeUintegerAccessor (&newradioPointToPointEpcHelper::m_x2LinkMtu),
                   MakeUintegerChecker<uint16_t> ())
  ;
  return tid;
}
...
void
newradioPointToPointEpcHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
newradioPointToPointEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
...
void
newradioPointToPointEpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
  ...
  Ptr<newradioEnbNetDevice> enb1newradioDev = enb1->GetDevice (0)->GetObject<newradioEnbNetDevice> ();
  Ptr<LteEnbNetDevice> enb1LteDev = enb1->GetDevice (0)->GetObject<LteEnbNetDevice> ();
  // we may have a LTE or a newradio eNB
  uint16_t enb1CellId = 0;
  if (enb1newradioDev != 0)
    {
      enb1CellId = enb1newradioDev->GetCellId ();
      NS_LOG_INFO ("newradioEnbNetDevice #1 = " << enb1newradioDev << " - CellId = " << enb1CellId);
    }
  ...
  Ptr<newradioEnbNetDevice> enb2newradioDev = enb2->GetDevice (0)->GetObject<newradioEnbNetDevice> ();
  Ptr<LteEnbNetDevice> enb2LteDev = enb2->GetDevice (0)->GetObject<LteEnbNetDevice> ();
  // we may have a LTE or a newradio eNB
  uint16_t enb2CellId = 0;
  if (enb2newradioDev != 0)
    {
      enb2CellId = enb2newradioDev->GetCellId ();
      NS_LOG_INFO ("newradioEnbNetDevice #2 = " << enb2newradioDev << " - CellId = " << enb2CellId);
      enb2newradioDev->GetRrc ()->AddX2Neighbour (enb1CellId);
    }
  ...
  if (enb1newradioDev != 0)
    {
      enb1newradioDev->GetRrc ()->AddX2Neighbour (enb2CellId);
    }
  else if (enb1LteDev != 0)
    {
...
void
newradioPointToPointEpcHelper::AddUe (Ptr<NetDevice> ueDevice, uint64_t imsi)
{
...
uint8_t
newradioPointToPointEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
  ...
  Ptr<newradio::newradioUeNetDevice> ueLteDevice = ueDevice->GetObject<newradio::newradioUeNetDevice> ();
  if (ueLteDevice)
    {
...
uint8_t
newradioPointToPointEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, Ptr<EpcUeNas> ueNas, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
  NS_LOG_FUNCTION (this << ueDevice << imsi);
...
Ptr<Node>
newradioPointToPointEpcHelper::GetPgwNode ()
{
  return m_sgwPgw;
}
...
Ptr<Node>
newradioPointToPointEpcHelper::GetMmeNode ()
{
  return m_mmeNode;
}

Ipv4InterfaceContainer
newradioPointToPointEpcHelper::AssignUeIpv4Address (NetDeviceContainer ueDevices)
{
  return m_uePgwAddressHelper.Assign (ueDevices);
}

Ipv6InterfaceContainer
newradioPointToPointEpcHelper::AssignUeIpv6Address (NetDeviceContainer ueDevices)
{
...
Ipv4Address
newradioPointToPointEpcHelper::GetUeDefaultGatewayAddress ()
{
...
Ipv6Address
newradioPointToPointEpcHelper::GetUeDefaultGatewayAddress6 ()
{
  // return the address of the tun device
