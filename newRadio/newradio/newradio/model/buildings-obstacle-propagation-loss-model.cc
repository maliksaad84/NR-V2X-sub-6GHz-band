      ...
      if (nlosSamples == -1 || losSamples == -1)
        {
          if (los)
            {
              loss = newradioLosLoss (a,b);
            }
          else
            {
              loss = newradioNlosLoss (a,b);
            }
        }
      else
        {
          if (!los && nlosSamples < g_nlosSamplesTrace)               // I am in NLOS and in the 'drop phase'
            {
              loss_inner = newradioLosLoss (a,b);
            ...
            {
              loss_inner = newradioLosLoss (a,b);                  // -----------------_> CHECK
...
double
BuildingsObstaclePropagationLossModel::newradioLosLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
...
double
BuildingsObstaclePropagationLossModel::newradioNlosLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  double distance = a->GetDistanceFrom (b);
...
void
BuildingsObstaclePropagationLossModel::SetBeamforming (Ptr<newradioBeamforming> beamforming)
{
  m_beamforming = beamforming;
}

void
BuildingsObstaclePropagationLossModel::SetLosTracker (Ptr<newradioLosTracker> losTracker)
{
  m_losTracker = losTracker;       // use m_losTracker in the class BuildingsObstaclePropagationLossModel
}

void
BuildingsObstaclePropagationLossModel::SetConfigurationParameters (Ptr<newradioPhyMacCommon> ptrConfig)
{
