#include "newradio-propagation-loss-model.h"
#include <ns3/log.h>
...
// ------------------------------------------------------------------------- //
NS_LOG_COMPONENT_DEFINE ("newradioPropagationLossModel");

namespace ns3 {

namespace newradio {

NS_OBJECT_ENSURE_REGISTERED (newradioPropagationLossModel);

TypeId
newradioPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::newradioPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .AddConstructor<newradioPropagationLossModel> ()
    .AddAttribute ("MinLoss",
                   "The minimum value (dB) of the total loss, used at short ranges. Note: ",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&newradioPropagationLossModel::SetMinLoss,
                                       &newradioPropagationLossModel::GetMinLoss),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("ChannelStates",
                   "'l' for LOS, 'n' for NLOS, 'o' for outage, 'a' for all",
                   StringValue ("a"),
                   MakeStringAccessor (&newradioPropagationLossModel::m_channelStates),
                   MakeStringChecker ())
    .AddAttribute ("LossFixedDb",
                   "",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&newradioPropagationLossModel::m_lossFixedDb),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("FixedLossTst",
                   "",
                   BooleanValue (false),
                   MakeBooleanAccessor (&newradioPropagationLossModel::m_fixedLossTst),
                   MakeBooleanChecker ())
  ;
  return tid;
}

newradioPropagationLossModel::newradioPropagationLossModel ()
{
  m_channelScenarioMap.clear ();
}

void
newradioPropagationLossModel::SetMinLoss (double minLoss)
{
  m_minLoss = minLoss;
}
double
newradioPropagationLossModel::GetMinLoss (void) const
{
  return m_minLoss;
}

double
newradioPropagationLossModel::GetFrequency (void) const
{
  return m_frequency;
}

void newradioPropagationLossModel::SetLossFixedDb (double loss)
{
  m_lossFixedDb = loss;
}

double
newradioPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                           Ptr<MobilityModel> a,
...
int64_t
newradioPropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}

void
newradioPropagationLossModel::UpDataScenarioMap ()
{
  m_channelScenarioMap.clear ();
  Simulator::Schedule (Seconds (1), &newradioPropagationLossModel::UpDataScenarioMap,this);
}

void
newradioPropagationLossModel::SetConfigurationParameters (Ptr<newradioPhyMacCommon> ptrConfig)
{
