...
NS_LOG_COMPONENT_DEFINE ("newradioChannelMatrix");

namespace newradio {

NS_OBJECT_ENSURE_REGISTERED (newradioChannelMatrix);

static const double CdfOfClusterNum[4] = {0.480, 0.761, 0.927, 1.0}; //Cdf of cluster number equal {1,2,3,4}


newradioChannelMatrix::newradioChannelMatrix ()
...
TypeId
newradioChannelMatrix::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::newradioChannelMatrix")
    .SetParent<Object> ()
    .AddAttribute ("ChunkWidth",
                   "Width of each chunk in Hz",
                   DoubleValue (13.889e6),
                   MakeDoubleAccessor (&newradioChannelMatrix::m_subBW),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("NumResourceBlocks",
                   "Number of resource blocks in one slot",
                   UintegerValue (4),
                   MakeUintegerAccessor (&newradioChannelMatrix::m_numRB),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("NumSubbandPerRB",
                   "Number of sub-bands in one resource block",
                   UintegerValue (18),
                   MakeUintegerAccessor (&newradioChannelMatrix::m_numSBPerRB),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
...
void
newradioChannelMatrix::DoDispose ()
{
  NS_LOG_FUNCTION (this);
}

void
newradioChannelMatrix::SetConfigurationParameters (Ptr<newradioPhyMacCommon> ptrConfig)
{
  m_phyMacConfig = ptrConfig;
}

Ptr<newradioPhyMacCommon>
newradioChannelMatrix::GetConfigurationParameters (void) const
{
...
void
newradioChannelMatrix::ConnectDevices (Ptr<NetDevice> dev1, Ptr<NetDevice> dev2)
{
...
void
newradioChannelMatrix::Initial (NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
...
Ptr<SpectrumValue>
newradioChannelMatrix::DoCalcRxPowerSpectralDensity (Ptr<const SpectrumValue> txPsd,
                                                   ...
                                                   Ptr<const MobilityModel> b) const
{
      ...
      if (mcRxUe != 0)
        {
          ...
          txAntennaArray = DynamicCast<AntennaArrayModel> (
              txEnb->GetPhy ()->GetDlSpectrumPhy ()->GetRxAntenna ());
          rxAntennaArray = DynamicCast<AntennaArrayModel> (
              mcRxUe->GetnewradioPhy ()->GetDlSpectrumPhy ()->GetRxAntenna ());
        }
    }
  else if (txEnb == 0 && rxUe == 0 )
    {
          ...
          if (mcTxUe != 0)
            {
              ...
              txAntennaArray = DynamicCast<AntennaArrayModel> (
                  mcTxUe->GetnewradioPhy ()->GetDlSpectrumPhy ()->GetRxAntenna ());
              rxAntennaArray = DynamicCast<AntennaArrayModel> (
                  rxEnb->GetPhy ()->GetDlSpectrumPhy ()->GetRxAntenna ());
            }
        }
  ...
  Ptr<newradioBeamFormingParams> bfParams = Create<newradioBeamFormingParams> ();
...
/*
complexVector_t
newradioChannelMatrix::CalcBeamformingVector(complex2DVector_t spatialMatrix) const
{
...
complex2DVector_t
newradioChannelMatrix::GenSpatialMatrix (std::vector<uint16_t> cluster, Angles angle, uint16_t* antennaNum) const
{
...
complexVector_t
newradioChannelMatrix::GenSinglePath (double hAngle, double vAngle, uint16_t* antennaNum) const
{
  NS_LOG_FUNCTION (this);
  complexVector_t singlePath;
...
Ptr<SpectrumValue>
newradioChannelMatrix::GetChannelGain (Ptr<const SpectrumValue> txPsd, Ptr<newradioBeamFormingParams> bfParams, double speed) const
{
  NS_LOG_FUNCTION (this);
...
double
newradioChannelMatrix::GetSystemBandwidth () const
{
  double bw = 0.00;
...
Ptr<SpectrumValue>
newradioChannelMatrix::CalcRxPowerSpectralDensity (Ptr<const SpectrumValue> txPsd,
                                                 Ptr<const MobilityModel> a,
                                                 Ptr<const MobilityModel> b) const
{
