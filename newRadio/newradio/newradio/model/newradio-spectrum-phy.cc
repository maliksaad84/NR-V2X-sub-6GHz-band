...
NS_LOG_COMPONENT_DEFINE ("newradioSpectrumPhy");

namespace newradio {

NS_OBJECT_ENSURE_REGISTERED (newradioSpectrumPhy);
...
newradioSpectrumPhy::newradioSpectrumPhy ()
  : m_cellId (0),
    m_state (IDLE),
    m_componentCarrierId (0)
{
  m_interferenceData = CreateObject<newradioInterference> ();
...
TypeId
newradioSpectrumPhy::GetTypeId (void)
{
  static TypeId
    tid =
    TypeId ("ns3::newradioSpectrumPhy")
    .SetParent<NetDevice> ()
    .AddTraceSource ("RxPacketTraceEnb",
                     "The no. of packets received and transmitted by the Base Station",
                     MakeTraceSourceAccessor (&newradioSpectrumPhy::m_rxPacketTraceEnb),
    ...
    .AddTraceSource ("RxPacketTraceUe",
                     ...
                     MakeTraceSourceAccessor (&newradioSpectrumPhy::m_rxPacketTraceUe),
    ...
    .AddAttribute ("DataErrorModelEnabled",
                   ...
                   BooleanValue (true),
                   MakeBooleanAccessor (&newradioSpectrumPhy::m_dataErrorModelEnabled),
                   MakeBooleanChecker ())

    .AddAttribute ("FileName",
                   "file name",
                   StringValue ("no"),
                   MakeStringAccessor (&newradioSpectrumPhy::m_fileName),
                   MakeStringChecker ())
  ;
...
void
newradioSpectrumPhy::DoDispose ()
{

}

void
newradioSpectrumPhy::Reset ()
{
...
void
newradioSpectrumPhy::ResetSpectrumModel ()
{
  m_rxSpectrumModel = 0;
}

void
newradioSpectrumPhy::SetDevice (Ptr<NetDevice> d)
{
  m_device = d;

  Ptr<newradioEnbNetDevice> enbNetDev =
    DynamicCast<newradioEnbNetDevice> (GetDevice ());
...
Ptr<NetDevice>
newradioSpectrumPhy::GetDevice () const
{
  return m_device;
}

void
newradioSpectrumPhy::SetMobility (Ptr<MobilityModel> m)
{
  m_mobility = m;
}

Ptr<MobilityModel>
newradioSpectrumPhy::GetMobility ()
{
  return m_mobility;
}

void
newradioSpectrumPhy::SetChannel (Ptr<SpectrumChannel> c)
{
  m_channel = c;
}

Ptr<const SpectrumModel>
newradioSpectrumPhy::GetRxSpectrumModel () const
{
  return m_rxSpectrumModel;
}

Ptr<AntennaModel>
newradioSpectrumPhy::GetRxAntenna ()
{
  return m_antenna;
}

void
newradioSpectrumPhy::SetAntenna (Ptr<AntennaModel> a)
{
  m_antenna = a;
}

void
newradioSpectrumPhy::SetState (State newState)
{
  ChangeState (newState);
}

void
newradioSpectrumPhy::ChangeState (State newState)
{
...
void
newradioSpectrumPhy::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
...
void
newradioSpectrumPhy::SetTxPowerSpectralDensity (Ptr<SpectrumValue> TxPsd)
{
  m_txPsd = TxPsd;
}

void
newradioSpectrumPhy::SetPhyRxDataEndOkCallback (newradioPhyRxDataEndOkCallback c)
{
...
void
newradioSpectrumPhy::SetPhyRxCtrlEndOkCallback (newradioPhyRxCtrlEndOkCallback c)
{
  m_phyRxCtrlEndOkCallback = c;
}

void
newradioSpectrumPhy::AddExpectedTb (uint16_t rnti, uint8_t ndi, uint32_t tbSize, uint8_t mcs,
...
void
newradioSpectrumPhy::SetPhyDlHarqFeedbackCallback (newradioPhyDlHarqFeedbackCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
newradioSpectrumPhy::SetPhyUlHarqFeedbackCallback (newradioPhyUlHarqFeedbackCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
newradioSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> params)
{

  NS_LOG_FUNCTION (this);

  Ptr<newradioEnbNetDevice> EnbTx =
    DynamicCast<newradioEnbNetDevice> (params->txPhy->GetDevice ());
  Ptr<newradioEnbNetDevice> enbRx =
    DynamicCast<newradioEnbNetDevice> (GetDevice ());
  ...
  Ptr<newradioSpectrumSignalParametersDataFrame> newradioDataRxParams =
    DynamicCast<newradioSpectrumSignalParametersDataFrame> (params);

  if (newradioDataRxParams != 0)
    {
      ...
      else if ((rxMcUe != 0) && (rxMcUe->GetnewradioPhy (m_componentCarrierId)->IsReceptionEnabled () == false))
      ...
      if (isAllocated)
        {
          m_interferenceData->AddSignal (newradioDataRxParams->psd, newradioDataRxParams->duration);
          if (newradioDataRxParams->cellId == m_cellId)
            {
              //m_interferenceData->AddSignal (newradioDataRxParams->psd, newradioDataRxParams->duration);
              StartRxData (newradioDataRxParams);
            }
          /*
          else
          {
    ...
    {
      Ptr<newradioSpectrumSignalParametersDlCtrlFrame> DlCtrlRxParams =
        DynamicCast<newradioSpectrumSignalParametersDlCtrlFrame> (params);
      if (DlCtrlRxParams != 0)
        {
...
void
newradioSpectrumPhy::StartRxData (Ptr<newradioSpectrumSignalParametersDataFrame> params)
{
  ...
  Ptr<newradioEnbNetDevice> enbRx =
    DynamicCast<newradioEnbNetDevice> (GetDevice ());
  Ptr<newradio::newradioUeNetDevice> ueRx =
    DynamicCast<newradio::newradioUeNetDevice> (GetDevice ());
            ...
            if (m_rxPacketBurstList.empty ())
              {
                ...
                m_endRxDataEvent = Simulator::Schedule (params->duration, &newradioSpectrumPhy::EndRxData, this);
              }
            else
              {
...
void
newradioSpectrumPhy::StartRxCtrl (Ptr<SpectrumSignalParameters> params)
{
  NS_LOG_FUNCTION (this);
            ...
            if (m_state == IDLE)
              {
                ...
                m_endRxDlCtrlEvent = Simulator::Schedule (params->duration, &newradioSpectrumPhy::EndRxCtrl, this);
                ChangeState (RX_CTRL);
              }
            else
              {
...
newradioSpectrumPhy::EndRxData ()
{
  ...
  Ptr<newradioEnbNetDevice> enbRx = DynamicCast<newradioEnbNetDevice> (GetDevice ());
  Ptr<newradio::newradioUeNetDevice> ueRx = DynamicCast<newradio::newradioUeNetDevice> (GetDevice ());
  ...
  while (itTb != m_expectedTbs.end ())
    {
      if ((m_dataErrorModelEnabled)&&(m_rxPacketBurstList.size () > 0))
        {
          newradioHarqProcessInfoList_t harqInfoList;
          uint8_t rv = 0;
          ...
          newradioTbStats_t tbStats = newradioMiErrorModel::GetTbDecodificationStats (m_sinrPerceived,
          ...
          if (itTb != m_expectedTbs.end ())
            {
              ...
              if (enbRx)
              ...
              else if (rxMcUe)
                {
                  ...
                  if (mcUe != 0)
                    {
                      Ptr<newradioEnbNetDevice> newradioEnb = mcUe->GetnewradioTargetEnb ();
                      if (newradioEnb != 0)
                        {
                          //traceParams.m_cellId = newradioEnb->GetCellId(); //now m_cellId is set correctly
                        }
                    }
...
bool
newradioSpectrumPhy::StartTxDataFrames (Ptr<PacketBurst> pb, std::list<Ptr<newradioControlMessage> > ctrlMsgList, Time duration, uint8_t slotInd)
{
  switch (m_state)
    {
    case RX_DATA:
    ...
    case IDLE:
      {
        ...
        m_state = TX;
        Ptr<newradioSpectrumSignalParametersDataFrame> txParams = Create<newradioSpectrumSignalParametersDataFrame> ();
...
//		Ptr<newradioEnbNetDevice> enbTx =
//					DynamicCast<newradioEnbNetDevice> (GetDevice ());
//		Ptr<newradioUeNetDevice> ueTx =
//					DynamicCast<newradioUeNetDevice> (GetDevice ());
//		if (enbTx)
//		{
...
//		}
        ...
        m_endTxEvent = Simulator::Schedule (duration, &newradioSpectrumPhy::EndTx, this);
      }
      break;
    default:
...
bool
newradioSpectrumPhy::StartTxDlControlFrames (std::list<Ptr<newradioControlMessage> > ctrlMsgList, Time duration)
{
    ...
    case IDLE:
      {
        ...
        Ptr<newradioSpectrumSignalParametersDlCtrlFrame> txParams = Create<newradioSpectrumSignalParametersDlCtrlFrame> ();
        txParams->duration = duration;
...
Ptr<SpectrumChannel>
newradioSpectrumPhy::GetSpectrumChannel ()
{
  return m_channel;
}

void
newradioSpectrumPhy::SetCellId (uint16_t cellId)
{
  m_cellId = cellId;
}

void
newradioSpectrumPhy::SetComponentCarrierId (uint8_t componentCarrierId)
{
...
void
newradioSpectrumPhy::AddDataPowerChunkProcessor (Ptr<newradioChunkProcessor> p)
{
  m_interferenceData->AddPowerChunkProcessor (p);
}

void
newradioSpectrumPhy::AddDataSinrChunkProcessor (Ptr<newradioChunkProcessor> p)
{
  m_interferenceData->AddSinrChunkProcessor (p);
}

void
newradioSpectrumPhy::UpdateSinrPerceived (const SpectrumValue& sinr)
{
...
void
newradioSpectrumPhy::SetHarqPhyModule (Ptr<newradioHarqPhy> harq)
{
