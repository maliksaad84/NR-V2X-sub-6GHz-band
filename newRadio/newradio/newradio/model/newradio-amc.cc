...
#include <ns3/math.h>
#include "ns3/enum.h"
#include "newradio-mi-error-model.h"

NS_LOG_COMPONENT_DEFINE ("newradioAmc");

namespace ns3 {

namespace newradio {

NS_OBJECT_ENSURE_REGISTERED (newradioAmc);
...
newradioAmc::newradioAmc ()
{
  NS_LOG_ERROR ("This construcor should not be invoked");
}

newradioAmc::newradioAmc (Ptr<newradioPhyMacCommon> ConfigParams)
...
TypeId
newradioAmc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::newradioAmc")
    .SetParent<Object> ()
    .AddConstructor<newradioAmc> ()
    .AddAttribute ("Ber",
                   "The requested BER in assigning MCS (default is 0.00005).",
                   DoubleValue (0.00005),
                   MakeDoubleAccessor (&newradioAmc::m_ber),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("AmcModel",
                   "AMC model used to assign CQI",
                   EnumValue (newradioAmc::MiErrorModel),
                   MakeEnumAccessor (&newradioAmc::m_amcModel),
                   MakeEnumChecker (newradioAmc::MiErrorModel, "Vienna",
                                    newradioAmc::PiroEW2010, "PiroEW2010"))
  ;
  return tid;
}

int
newradioAmc::GetMcsFromCqi (int cqi)
{
  NS_LOG_FUNCTION (cqi);
...
int
newradioAmc::GetTbSizeFromMcs (unsigned mcs, unsigned nprb)
{
  NS_LOG_FUNCTION (mcs);
...
int
newradioAmc::GetTbSizeFromMcsSymbols (unsigned mcs, unsigned nsymb)
{
  NS_LOG_FUNCTION (mcs);
  NS_ASSERT_MSG (mcs < 29, "MCS=" << mcs);
...
int
newradioAmc::GetNumSymbolsFromTbsMcs (unsigned tbSize, unsigned mcs)
{
  NS_LOG_FUNCTION (mcs);
  NS_ASSERT_MSG (mcs < 29, "MCS=" << mcs);
  //unsigned itb = McsToItbs[mcs];
...
std::vector<int>
newradioAmc::CreateCqiFeedbacks (const SpectrumValue& sinr, uint8_t rbgSize)
{
      ...
      for (it = sinr.ConstValuesBegin (); it != sinr.ConstValuesEnd (); it++)
        {
          ...
          if ((rbId % rbgSize == 0)||((it + 1) == sinr.ConstValuesEnd ()))
            {
              uint8_t mcs = 0;
              newradioTbStats_t tbStats;
              while (mcs <= 28)
                {
                  newradioHarqProcessInfoList_t harqInfoList;
                  tbStats = newradioMiErrorModel::GetTbDecodificationStats (sinr, rbgMap, GetTbSizeFromMcs (mcs, rbgSize / 18) / 8, mcs, harqInfoList);
...
std::vector<int>
newradioAmc::CreateCqiFeedbacksTdma (const SpectrumValue& sinr, uint8_t numSym)
{
  NS_LOG_FUNCTION (this);
      ...
      for (it = sinr.ConstValuesBegin (); it != sinr.ConstValuesEnd (); it++)
        {
          ...
          while (mcs <= 28)
            {
              newradioHarqProcessInfoList_t harqInfoList;
              tbStats = newradioMiErrorModel::GetTbDecodificationStats (sinr, chunkMap, GetTbSizeFromMcsSymbols (mcs, numSym) / 8, mcs, harqInfoList);
              if (tbStats.tbler > 0.1)
                {
                  break;
                }
...
int
newradioAmc::CreateCqiFeedbackWbTdma (const SpectrumValue& sinr, uint8_t numSym, uint32_t tbSize, int &mcs)
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_amcModel == PiroEW2010)
  ...
  else if (m_amcModel == MiErrorModel)
    {
      ...
      while (mcs <= 28)
        {
          newradioHarqProcessInfoList_t harqInfoList;
          tbStats = newradioMiErrorModel::GetTbDecodificationStats (sinr, chunkMap, tbSize, mcs, harqInfoList);
          if (tbStats.tbler > 0.1)
            {
              break;
            }
...
//		newradioHarqProcessInfoList_t harqInfoList;
//		newradioTbStats_t tbStatsFinal = newradioMiErrorModel::GetTbDecodificationStats (sinr, chunkMap, tbSize, mcs, harqInfoList);
...
int
newradioAmc::GetCqiFromSpectralEfficiency (double s)
{
  NS_LOG_FUNCTION (s);
...
int
newradioAmc::GetMcsFromSpectralEfficiency (double s)
{
  NS_LOG_FUNCTION (s);
  NS_ASSERT_MSG (s >= 0.0, "negative spectral efficiency = " << s);
  int mcs = 0;
