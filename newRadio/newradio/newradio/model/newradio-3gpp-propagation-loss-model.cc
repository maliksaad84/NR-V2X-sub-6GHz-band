#include "newradio-3gpp-propagation-loss-model.h"
#include <ns3/log.h>
...
// ------------------------------------------------------------------------- //
NS_LOG_COMPONENT_DEFINE ("newradio3gppPropagationLossModel");

NS_OBJECT_ENSURE_REGISTERED (newradio3gppPropagationLossModel);

TypeId
newradio3gppPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::newradio3gppPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .AddConstructor<newradio3gppPropagationLossModel> ()
    .AddAttribute ("MinLoss",
                   "The minimum value (dB) of the total loss, used at short ranges.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&newradio3gppPropagationLossModel::SetMinLoss,
                                       &newradio3gppPropagationLossModel::GetMinLoss),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("ChannelCondition",
                   "'l' for LOS, 'n' for NLOS, 'a' for all",
                   StringValue ("a"),
                   MakeStringAccessor (&newradio3gppPropagationLossModel::m_channelConditions),
                   MakeStringChecker ())
    .AddAttribute ("Scenario",
                   ...
                   StringValue ("RMa"),
                   MakeStringAccessor (&newradio3gppPropagationLossModel::m_scenario),
                   MakeStringChecker ())
    .AddAttribute ("OptionalNlos",
                   "Use the optional NLoS propagation loss model",
                   BooleanValue (false),
                   MakeBooleanAccessor (&newradio3gppPropagationLossModel::m_optionNlosEnabled),
                   MakeBooleanChecker ())
    .AddAttribute ("Shadowing",
                   "Enable shadowing effect",
                   BooleanValue (true),
                   MakeBooleanAccessor (&newradio3gppPropagationLossModel::m_shadowingEnabled),
                   MakeBooleanChecker ())
    .AddAttribute ("InCar",
                   "If inside a vehicle, car penetration loss should be added to propagation loss",
                   BooleanValue (false),
                   MakeBooleanAccessor (&newradio3gppPropagationLossModel::m_inCar),
                   MakeBooleanChecker ())
  ;
  return tid;
}

newradio3gppPropagationLossModel::newradio3gppPropagationLossModel ()
{
...
void
newradio3gppPropagationLossModel::SetMinLoss (double minLoss)
{
  m_minLoss = minLoss;
}
double
newradio3gppPropagationLossModel::GetMinLoss (void) const
{
  return m_minLoss;
}

double
newradio3gppPropagationLossModel::GetFrequency (void) const
{
  return m_frequency;
}
...
double
newradio3gppPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                               Ptr<MobilityModel> a,
...
double
newradio3gppPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  Ptr<MobilityModel> ueMob, enbMob;
  if ((DynamicCast<newradio::newradioUeNetDevice> (a->GetObject<Node> ()->GetDevice (0)) != 0)
      || (DynamicCast<McUeNetDevice> (a->GetObject<Node> ()->GetDevice (0)) != 0))
    {
      if (DynamicCast<newradioEnbNetDevice> (b->GetObject<Node> ()->GetDevice (0)) != 0)
        {
          ueMob = a;
          enbMob = b;
        }
...
int64_t
newradio3gppPropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}

void
newradio3gppPropagationLossModel::UpdateConditionMap (Ptr<MobilityModel> a, Ptr<MobilityModel> b, channelCondition cond) const
{
...
char
newradio3gppPropagationLossModel::GetChannelCondition (Ptr<MobilityModel> a, Ptr<MobilityModel> b)
{
  channelConditionMap_t::const_iterator it;
  it = m_channelConditionMap.find (std::make_pair (a,b));
  if (it == m_channelConditionMap.end ())
...
std::string
newradio3gppPropagationLossModel::GetScenario ()
{
  return m_scenario;
}

void
newradio3gppPropagationLossModel::SetConfigurationParameters (Ptr<newradioPhyMacCommon> ptrConfig)
{
