...
NS_LOG_COMPONENT_DEFINE ("newradioBeamforming");

NS_OBJECT_ENSURE_REGISTERED (newradioBeamforming);
...
newradioBeamforming::newradioBeamforming (uint32_t enbAntenna, uint32_t ueAntenna)
  : m_pathNum (20),
...
TypeId
newradioBeamforming::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::newradioBeamforming")
    .SetParent<Object> ()
    .AddAttribute ("LongTermUpdatePeriod",
                   ...
                   MakeTimeAccessor (&newradioBeamforming::m_longTermUpdatePeriod),
                   MakeTimeChecker ())
    .AddAttribute ("SmallScaleFading",
                   "Enable small scale fading",
                   BooleanValue (true),
                   MakeBooleanAccessor (&newradioBeamforming::m_smallScale),
                   MakeBooleanChecker ())
    .AddAttribute ("FixSpeed",
                   "Set a fixed speed (even if constant position) so doppler > 0 for testing",
                   BooleanValue (false),
                   MakeBooleanAccessor (&newradioBeamforming::m_fixSpeed),
                   MakeBooleanChecker ())
    .AddAttribute ("UeSpeed",
                   "UE speed (m/s) for fixed speed test",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&newradioBeamforming::m_ueSpeed),
                   MakeDoubleChecker<double> ())
  ;
  return tid;
}

newradioBeamforming::~newradioBeamforming ()
{

}

void
newradioBeamforming::DoDispose ()
{
  NS_LOG_FUNCTION (this);
}

void
newradioBeamforming::SetConfigurationParameters (Ptr<newradioPhyMacCommon> ptrConfig)
{
  m_phyMacConfig = ptrConfig;
}

Ptr<newradioPhyMacCommon>
newradioBeamforming::GetConfigurationParameters (void) const
{
  return m_phyMacConfig;
}

std::complex<double>
newradioBeamforming::ParseComplex (std::string strCmplx)
{
  double re = 0.00;
  double im = 0.00;
  size_t findj = 0;
...
void
newradioBeamforming::LoadSmallScaleFading ()
{
  std::string filename = "src/newradio/model/BeamFormingMatrix/SmallScaleFading.txt";
...
void
newradioBeamforming::LoadEnbAntenna ()
{
  std::string filename = "src/newradio/model/BeamFormingMatrix/TxAntenna.txt";
...
void
newradioBeamforming::LoadUeAntenna ()
{
  std::string filename = "src/newradio/model/BeamFormingMatrix/RxAntenna.txt";
...
void
newradioBeamforming::LoadEnbSpatialSignature ()
{
  std::string filename = "src/newradio/model/BeamFormingMatrix/TxSpatialSigniture.txt";
...
void
newradioBeamforming::LoadUeSpatialSignature ()
{
  std::string strFilename = "src/newradio/model/BeamFormingMatrix/RxSpatialSigniture.txt";
...
void
newradioBeamforming::Initial (NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
  ...
  //    Ptr<newradioUeNetDevice> UeDev = (*i)->GetObject<newradioUeNetDevice> ();
  //    if (UeDev != 0) { // It actually is a newradioUeNetDevice
  ...
  Simulator::Schedule (m_longTermUpdatePeriod, &newradioBeamforming::Initial,this,ueDevices,enbDevices);
  ...
  Simulator::Schedule (MicroSeconds (m_nextLongTermUpdate->GetValue()), &newradioBeamforming::Initial,this,ueDevices,enbDevices);*/
}


void
newradioBeamforming::SetChannelMatrix (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  ...
  //Simulator::Schedule (Seconds (m_longTermUpdatePeriod), &newradioBeamforming::SetChannelMatrix,this,ueDevice,enbDevice);
}

void
newradioBeamforming::SetBeamformingVector (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  key_t key = std::make_pair (ueDevice,enbDevice);
...
complexVector_t*
newradioBeamforming::GetLongTermFading (Ptr<BeamformingParams> bfParams) const
{
...
Ptr<SpectrumValue>
newradioBeamforming::GetChannelGainVector (Ptr<const SpectrumValue> txPsd, Ptr<BeamformingParams> bfParams, double speed) const
{
  NS_LOG_FUNCTION (this);
...
Ptr<SpectrumValue>
newradioBeamforming::CalcRxPowerSpectralDensity (Ptr<const SpectrumValue> txPsd,
...
Ptr<SpectrumValue>
newradioBeamforming::DoCalcRxPowerSpectralDensity (Ptr<const SpectrumValue> txPsd,
                                                 Ptr<const MobilityModel> a,
                                                 Ptr<const MobilityModel> b) const
{
  bool downlink;
...
//		std::cout << std::endl;
  ...
  Ptr<newradio::newradioUeNetDevice> ueMmw = ueDevice->GetObject<newradio::newradioUeNetDevice> ();
...
double
newradioBeamforming::GetSystemBandwidth () const
{
  double bw = 0.00;
...
void
newradioBeamforming::UpdateMatrices (bool update)
{
  m_update = update;
}

antennaPair
newradioBeamforming::GetUeEnbAntennaPair (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice) const
{
  Ptr<newradioEnbNetDevice> EnbDev =
    DynamicCast<newradioEnbNetDevice> (enbDevice);
  Ptr<newradio::newradioUeNetDevice> UeDev = ueDevice->GetObject<newradio::newradioUeNetDevice> ();
  // DynamicCast<newradioUeNetDevice> (*i);
  Ptr<newradioUePhy> uePhy;
  Ptr<newradioEnbPhy> enbPhy = EnbDev->GetPhy ();
  if (UeDev != 0)         // It actually is a newradioUeNetDevice
    {
      uePhy = UeDev->GetPhy ();
    }
