#include "newradio-vehicular-spectrum-propagation-loss-model.h"
#include <ns3/log.h>
#include <ns3/math.h>
...
NS_LOG_COMPONENT_DEFINE ("newradioVehicularSpectrumPropagationLossModel");

NS_OBJECT_ENSURE_REGISTERED (newradioVehicularSpectrumPropagationLossModel);
...
newradioVehicularSpectrumPropagationLossModel::newradioVehicularSpectrumPropagationLossModel ()
{
...
TypeId
newradioVehicularSpectrumPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::newradioVehicularSpectrumPropagationLossModel")
    .SetParent<SpectrumPropagationLossModel> ()
    .AddConstructor<newradioVehicularSpectrumPropagationLossModel> ()
    .AddAttribute ("Frequency",
                   "Operating frequency in Hz",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&newradioVehicularSpectrumPropagationLossModel::m_frequency),
    ...
    .AddAttribute ("UpdatePeriod",
                   ...
                   MakeTimeAccessor (&newradioVehicularSpectrumPropagationLossModel::m_updatePeriod),
                   MakeTimeChecker ())
    .AddAttribute ("Blockage",
                   "Enable blockage model A (sec 7.6.4.1)",
                   BooleanValue (false),
                   MakeBooleanAccessor (&newradioVehicularSpectrumPropagationLossModel::m_blockage),
                   MakeBooleanChecker ())
    .AddAttribute ("NumNonselfBlocking",
                   "number of non-self-blocking regions",
                   IntegerValue (4),
                   MakeIntegerAccessor (&newradioVehicularSpectrumPropagationLossModel::m_numNonSelfBloking),
                   MakeIntegerChecker<uint16_t> ())
    .AddAttribute ("BlockerSpeed",
                   "The speed of moving blockers, the unit is m/s",
                   DoubleValue (1),
                   MakeDoubleAccessor (&newradioVehicularSpectrumPropagationLossModel::m_blockerSpeed),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("PortraitMode",
                   "true for portrait mode, false for landscape mode",
                   BooleanValue (true),
                   MakeBooleanAccessor (&newradioVehicularSpectrumPropagationLossModel::m_portraitMode),
                   MakeBooleanChecker ())
    .AddAttribute ("OxygenAbsorption",
                   "true for considering oxygen absortion, false for not considering it",
                   BooleanValue (true),
                   MakeBooleanAccessor (&newradioVehicularSpectrumPropagationLossModel::m_oxygenAbsorption),
                   MakeBooleanChecker ())
    .AddAttribute ("O2I",
                   "Outdoor to Indoor channel state",
                   BooleanValue (false),
                   MakeBooleanAccessor (&newradioVehicularSpectrumPropagationLossModel::m_o2i),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
void
newradioVehicularSpectrumPropagationLossModel::DoDispose ()
{
  NS_LOG_FUNCTION (this);
}

void
newradioVehicularSpectrumPropagationLossModel::AddDevice (Ptr<NetDevice> dev, Ptr<newradioVehicularAntennaArrayModel> antenna)
{
  NS_ASSERT_MSG (m_deviceAntennaMap.find (dev) == m_deviceAntennaMap.end (), "Device is already present in the map");
  m_deviceAntennaMap.insert (std::pair <Ptr<NetDevice>, Ptr<newradioVehicularAntennaArrayModel>> (dev, antenna));
}

Ptr<SpectrumValue>
newradioVehicularSpectrumPropagationLossModel::DoCalcRxPowerSpectralDensity (Ptr<const SpectrumValue> txPsd,
                                                 ...
                                                 Ptr<const MobilityModel> b) const
{
  ...
  Ptr<newradioVehicularAntennaArrayModel> txAntennaArray = m_deviceAntennaMap.at (txDevice);
  ...
  Ptr<newradioVehicularAntennaArrayModel> rxAntennaArray = m_deviceAntennaMap.at (rxDevice);
  ...
  char condition;
  if (DynamicCast<newradioVehicularPropagationLossModel> (m_3gppPathloss) != 0)
    {
      condition = m_3gppPathloss->GetObject<newradioVehicularPropagationLossModel> ()
          ...
          if (m_updatePeriod.GetMilliSeconds () > 0)
            {
              ...
              Simulator::Schedule (m_updatePeriod, &newradioVehicularSpectrumPropagationLossModel::DeleteChannel,this,a,b);
            }
        }
...
Ptr<SpectrumValue>
newradioVehicularSpectrumPropagationLossModel::CalBeamformingGain (Ptr<const SpectrumValue> txPsd, Ptr<Params3gpp> params,
...
double
newradioVehicularSpectrumPropagationLossModel::GetOxygenLoss (double f, double dist3D, double tau, double tauDelta) const
{
...
void
newradioVehicularSpectrumPropagationLossModel::SetPathlossModel (Ptr<PropagationLossModel> pathloss)
{
  m_3gppPathloss = pathloss;
  if (DynamicCast<newradioVehicularPropagationLossModel> (m_3gppPathloss) != 0)
    {
      m_scenario = m_3gppPathloss->GetObject<newradioVehicularPropagationLossModel> ()->GetScenario ();
    }
  // else if (DynamicCast<newradio3gppBuildingsPropagationLossModel> (m_3gppPathloss) != 0)
  //   {
...
complexVector_t
newradioVehicularSpectrumPropagationLossModel::CalLongTerm (Ptr<Params3gpp> params) const
{
...
Ptr<ParamsTable>
newradioVehicularSpectrumPropagationLossModel::Get3gppTable (char condition, bool o2i, double hBS, double hUT, double distance2D) const
{
...
void
newradioVehicularSpectrumPropagationLossModel::DeleteChannel (Ptr<const MobilityModel> a, Ptr<const MobilityModel> b) const
{
  NS_LOG_FUNCTION (this);
...
Ptr<Params3gpp>
newradioVehicularSpectrumPropagationLossModel::GetNewChannel (Ptr<ParamsTable>  table3gpp, Vector locUT, char condition, bool o2i,
                                  Ptr<newradioVehicularAntennaArrayModel> txAntenna, Ptr<newradioVehicularAntennaArrayModel> rxAntenna,
...
Ptr<Params3gpp>
newradioVehicularSpectrumPropagationLossModel::UpdateChannel (Ptr<Params3gpp> params3gpp, Ptr<ParamsTable>  table3gpp,
                                  Ptr<newradioVehicularAntennaArrayModel> txAntenna, Ptr<newradioVehicularAntennaArrayModel> rxAntenna,
...
doubleVector_t
newradioVehicularSpectrumPropagationLossModel::CalAttenuationOfBlockage (Ptr<Params3gpp> params,
